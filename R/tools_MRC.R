...
get_z_index = function(x){
  ...
  # this arrives in the case of missing data in the t-1 and t+1 times 
  Z[!is.na(x) & is.na(Z)] = 0.5
  Z[x == 0] = NA
  
  # keep NA, NAs in the precipitations
  Z[is.na(x)] = NA
...
#' 
#' 
#' @return a list of data frames with different parameters of MRC as estimated on precipitation data.  
#' 
#' 
#' 
#' @export
estimate_params_MRC = function(vecPrecip,
  ...
  if(all(diff_time == resVecPrecip) & resVecPrecip == 10){ # if time difference for the whole time series is 10 minutes
  ...
  } else if (resVecPrecip == 10) { # make sure that each "day" has 128 observations
