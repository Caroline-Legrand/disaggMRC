...
#' 
#' Estimate scaling parameters of a MRC model, where the cascade weights are 
...
#' temporal aggregation level, precipitation intensity class and asymmetry index class z. 
#' In the last step, scaling models are fitted on estimated MRC parameter by 
#' the method of linear least square. Plots showing estimated parameters and their scaling models 
#' are also returned   
#' 
...
#' @param threshold for weight discarding in all estimation procedure, in mm. Default 0 mm.
#' @param threshold_alpha_int for discarding alpha estimated at small intensities, in mm. Default 0.8 mm.
#' @param threshold_asymm for discarding weights of small intensities for estimation of asymmetry parameters, in mm. Default 0.8 mm.
#' 
...
get_scaling_params_Intensity_aggLevel = function(vecPrecip,
  ...
  ggplot_px_params_aggLevel = ggplot(int_pars_px)+
    ...
    labs(title = "Px params as function of temporal scale", 
  ...
  # predict alpha star values by the model 
  predict_alpha_star = left_join(expand.grid(Intensity = vecIntensity, Season = vec.season.char), 
                                 params_alpha, by = "Season") %>%
  ...
  predict_phi = left_join(expand.grid(zIndex = seq(0,1,length.out = 30), Season = vec.season.char), 
                          params_asymm, by = "Season") %>%
    mutate(phi = get_phi_z(z = zIndex, nu = nu))
  
  predict_mean_weights = left_join(expand.grid(zIndex = seq(0,1,length.out = 30), Season = vec.season.char), 
                                   params_asymm, by = "Season") %>%
...
#' 
#' Estimate scaling parameters of a MRC model, where the cascade weights are 
#' considered to be dependent on the intensity of precipitation that has to be disaggregated.
#' In the first step observed weights are calculated for each temporal aggregation level. 
#' Also at this stage, for each observed precipitation intensity, its index z is calculated.  
#' Second, MRC parameters are estimated by considering a dependency to 
#' temporal aggregation level, precipitation intensity class and asymmetry index class z. 
#' In the last step, scaling models are fitted on estimated MRC parameter by 
#' the method of linear least square. Plots showing estimated paramters and their scaling models 
#' are also returned   
#' 
#' @param vecPrecip a vector of observed precipitations 
...
#' @param threshold for weight discarding in all estimation procedure, in mm. Default 0.8 mm. 
#' @param threshold_alpha_int for discarding alpha estimated at small intensities, in mm. Default 0.8 mm. 
#' @param threshold_asymm for discarding weights of small intensities for estimation of asymmetry parameters, in mm. Default 0.8 mm.
#' 
#' 
...
get_scaling_params_Intensity = function(vecPrecip,
  ...
  predict_alpha_int = left_join(expand.grid(Intensity = vecIntensity, Season = vec.season.char), 
...
disaggregate_precip_MRC_Intensity_aggLevel = function(vecPrecip_target,
  ...
  for (i_scenario in 1:nb_scenarios){
    ...
    while(i_aggLevel > res_fine_aggLevel){
      ...
      data_current_params = left_join(data_current, params_scaling,
                                      by = "Season")
