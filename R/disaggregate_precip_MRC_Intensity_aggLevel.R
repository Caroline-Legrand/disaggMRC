...
#' where the dependence on temporal aggregation level and precipitation intensity is taken into account
#' 
  ...
  while (i_aggLevel > res_stop_disagg) {
    ...
    data_current_params = data_current_params %>% 
      mutate(Px = get_Px_Intensity_aggLevel(Intensity, i_aggLevel/1440, a_mu, b_mu, a_sigma, b_sigma),
             alpha = get_alpha_aggLevel(alpha0, H, res_aggLevel = i_aggLevel/1440)*alpha_star_Intensity(Intensity, c0, c1, c2),
